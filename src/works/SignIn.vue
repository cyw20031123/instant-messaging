<template>
	<div class="page-section container-fluid text-center SignIn">
		<div class="LoginInterface">
			<div class="button">
				<div @click="pattern = true" :style="pattern ? 'color:lightskyblue' : ''">
					登录
				</div>
				<div @click="pattern = false" :style="!pattern ? 'color:lightskyblue' : ''">
					注册
				</div>
			</div>
...
<script>
	...
	export default {
		...
		created() {
			...
			if (currentUser) {
				this.$emit("up",[ "Chat",currentUser.attributes.username]);
			}
		},
	};
</script>
